...
def adjust_matrix(apparel_id, adjust_value, user_id):
    ...
    executeNonQuery("update prefers set rating = rating + %d where user_id = '%s' and label_id in (%s)" % (adjust_value, user_id, labels))
...
def add_user_item(img_path, labels):
    executeNonQuery("insert into apparel values(0, '%s', false)" % img_path)
    insertId = lastInsertedId()
    for val in labels:
        executeNonQuery("insert into has_labels values(%d, %d)" % (insertId, label_dict[val[0]]))

def get_wardrobe_items(user_folder):
    img_paths = paths.list_images(user_folder)
    img_paths = ",".join(["'"+x+"'" for x in img_paths])
    print("ImgPaths:" + img_paths)
    data = []
    if img_paths != "" and img_paths != None:
        data = getAllValues("select distinct(img_path),name from apparel a, has_labels h , label l where a.id = apparel_id and label_id = l.id and inactive=false and img_path in (%s)" % img_paths)
    print(data)
    return data

def remove_apparel(path):
    executeNonQuery("update apparel set inactive=true where img_path = '%s'" % path)